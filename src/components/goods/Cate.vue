...
export default {
  ...
  methods: {
    // 获取商品分类
    async getCateList () {
      const { data: res } = await this.$http.post('categories', this.$qs.stringify(this.queryInfo))
      if (res.status !== 200) {
    ...
    async getParentCateList () {
      const { data: res } = await this.$http.get('categories/2')
      if (res.status !== 200) {
    ...
    addCate () {
      this.$refs.addCateFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await this.$http.post('addCategory', this.$qs.stringify(this.addCateForm))
        if (res.status !== 200) {
