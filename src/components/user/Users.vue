        ...
        <div>
          ...
          <p>分配新角色：
            <el-select v-model="selectedRoleId" placeholder="请选择">
              <el-option clearable v-for="item in rolesList" :key="item.id" :label="item.roleName" :value="item.id">
              </el-option>
            </el-select>
          </p>
        </div>
...
export default {
  ...
  methods: {
    async getUserList () {
      const { data: res } = await this.$http.post('users', this.$qs.stringify(this.queryInfo))
      if (res.status !== 200) {
    ...
    async userStateChanged (user) {
      const { data: res } = await this.$http.put(`update/${user.userId}/state/${user.mg_state}`)
    ...
    editUserInfo () {
      this.$refs.editFormRef.validate(async valid => {
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put('edit/' + this.editForm.userId,
          {
    ...
    async deleteUserByUserId (id) {
      ...
      const { data: resp } = await this.$http.delete('delete/' + id)
    ...
    async saveRoleInfo () {
      ...
      const { data: res } = await this.$http.put(
        ...
        this.$qs.stringify({ rid: this.selectedRoleId })
      )
